...
class SupplierController extends Controller
{
    ...
    public function Bulkstore(Request $req)
    {
        $validate = $req->validate([
            ...
            "suppliers.*.name" => "required|string|min:3|max:255",
            "suppliers.*.email" => "required|email|unique:suppliers,email",
            "suppliers.*.address" => "required|string|min:3|max:255",
            "suppliers.*.website" => "required|string|url|max:255",
        ]);

        $suppliers = [];
        foreach ($validate['suppliers'] as $item) {
            $suppliers[] =   Supplier::create($item);
        }
